...
<div class="max-w-4xl mx-auto py-8">
        ...
        <div class="flex gap-3">
            @if($invoice->bill_type === 'estimation' && $invoice->payment_status !== 'converted')
                <form action="{{ route('admin.invoices.convert', $invoice->id) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" onclick="return confirm('Covert this estimation to a Tax Invoice?')" class="px-5 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-black text-xs uppercase tracking-widest hover:bg-emerald-500 hover:text-white transition-all shadow-lg flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>
                        Convert to Tax Invoice
                    </button>
                </form>
            @endif
            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $invoice->phone) }}?text={{ urlencode('Hello ' . $invoice->customer_name . ', here is your ' . ($invoice->bill_type === 'estimation' ? 'Estimation' : 'Tax Invoice') . ' from TC Service Center. Number: ' . $invoice->invoice_number . '. Amount: Rs ' . $invoice->total . '. Click to view: ' . route('admin.invoices.print', $invoice->id)) }}" target="_blank" class="px-5 py-2 rounded-xl bg-green-500/10 border border-green-500/20 text-green-400 hover:bg-green-500 hover:text-white font-black text-xs uppercase tracking-widest transition-all shadow-lg flex items-center gap-2">
            ...
            <a href="{{ route('admin.invoices.print', $invoice->id) }}?auto_print=1" target="_blank" class="px-5 py-2 rounded-xl bg-white/5 border border-white/10 text-white font-black text-xs uppercase tracking-widest hover:bg-white/10 transition-all shadow-lg flex items-center gap-2">
    ...
    <div id="invoice-paper" class="bg-white text-slate-900 rounded-[2rem] shadow-2xl overflow-hidden p-12 relative border border-slate-100 min-h-[1000px]">
        ...
        <div class="absolute top-0 right-0 p-12 opacity-[0.03] pointer-events-none">
            <h1 class="text-8xl font-black italic">{{ $invoice->bill_type === 'estimation' ? 'ESTIMATION' : 'INVOICE' }}</h1>
        </div>
        ...
        <div class="flex justify-between items-start mb-16 relative z-10">
            ...
            <div class="text-right">
                <div class="mb-6">
                    <span class="px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full border {{ $invoice->bill_type === 'estimation' ? 'bg-sky-50 text-sky-700 border-sky-100' : 'bg-indigo-50 text-indigo-700 border-indigo-100' }}">
                        {{ $invoice->bill_type === 'estimation' ? 'Estimation / Quotation' : 'Tax Invoice' }}
                    </span>
                </div>
                ...
                @if($invoice->valid_until)
                    <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">Valid Until: <span class="text-rose-500">{{ $invoice->valid_until->format('d M, Y') }}</span></p>
                @endif
            </div>
        </div>
