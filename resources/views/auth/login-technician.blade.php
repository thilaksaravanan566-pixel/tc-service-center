...
<head>
    ...
    @endif
    <title>Technician Terminal | {{ config('custom.company_name', 'Thambu Computers') }}</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;600;800&family=Spartan:wght@700;900&display=swap" rel="stylesheet">
    <style>
        body { font-family: 'Spartan', sans-serif; overflow: hidden; margin: 0; padding: 0; }
        .input-font { font-family: 'JetBrains Mono', monospace; }
        .ambient-bg {
        ...
        .illustration-panel {
            background: url('https://images.unsplash.com/photo-1555680202-c86f0e12f086?auto=format&fit=crop&q=80&w=1000') center center / cover no-repeat;
            position: relative;
        }
        .illustration-overlay { background: linear-gradient(180deg, rgba(2, 44, 34, 0.7) 0%, rgba(2, 44, 34, 0.95) 100%); position: absolute; inset: 0; }
        .glow-orb {
            position: absolute; top: 20%; left: 30%; width: 250px; height: 250px;
            background: rgba(16, 185, 129, 0.4); filter: blur(80px); border-radius: 50%;
            animation: pulseGlow 4s ease-in-out infinite alternate;
        }
        @keyframes pulseGlow { from { transform: scale(0.8); opacity: 0.6; } to { transform: scale(1.2); opacity: 1; } }
        .modern-input { background-color: #f0fdf4; transition: all 0.3s ease; }
        .modern-input:focus { background-color: #ffffff; border-color: #10b981; box-shadow: 0 0 0 4px rgba(16, 185, 129, 0.1); }
        .btn-gradient {
            background: linear-gradient(90deg, #047857 0%, #10b981 100%);
            transition: all 0.4s ease; background-size: 200% auto;
        }
        .btn-gradient:hover { background-position: right center; box-shadow: 0 10px 20px -5px rgba(16, 185, 129, 0.5); transform: translateY(-2px); }
    </style>
</head>
    ...
    <div class="floating-card bg-white w-full max-w-5xl rounded-3xl flex flex-col md:flex-row overflow-hidden min-h-[550px] relative z-10">
        <div class="hidden md:block w-1/2 illustration-panel">
            ...
            <div class="absolute inset-0 flex flex-col justify-end p-12 text-white z-10">
                ...
                <h3 class="text-3xl font-black mb-3 tracking-tighter leading-tight">Service Lab<br>Terminal</h3>
                <p class="text-xs font-semibold text-emerald-200 leading-relaxed uppercase tracking-wider mb-2">TC Hardware Engineering</p>
        ...
        <div class="w-full md:w-1/2 p-10 lg:p-16 flex flex-col justify-center bg-white relative">
            <h2 class="text-4xl font-black text-[#064e3b] mb-2 tracking-tighter">TECH LOGIN</h2>
            <p class="text-[11px] uppercase tracking-widest text-slate-400 mb-10 font-bold">Initialize lab session.</p>
            @if ($errors->any())
            ...
            <form method="POST" action="{{ url('/login') }}" class="space-y-5">
                ...
                <div class="relative group">
                    <div class="absolute inset-y-0 left-0 flex items-center pl-5 pointer-events-none text-slate-400 group-focus-within:text-[#10b981] transition-colors">
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required placeholder="technician_id@domain" class="modern-input input-font w-full border-transparent text-sm text-slate-800 font-semibold placeholder-slate-400 rounded-xl py-4 pl-12 pr-4 outline-none">
                </div>
                ...
                <div class="relative group">
                    ...
                    <input type="password" name="password" required placeholder="********" class="modern-input input-font w-full border-transparent text-sm text-slate-800 font-semibold placeholder-slate-400 rounded-xl py-4 pl-12 pr-4 outline-none">
                </div>

                <div class="flex items-center justify-between mt-2 pl-1">
                    <label class="flex items-center cursor-pointer group">
                        <input type="checkbox" name="remember" class="w-3 h-3 rounded border-slate-300 text-[#10b981] focus:ring-[#10b981] cursor-pointer bg-slate-100 outline-none hover:bg-white">
                        <span class="ml-2 text-[10px] font-bold uppercase tracking-wider text-slate-400 group-hover:text-slate-600 transition-colors">Remember Login</span>
                    </label>
                </div>
                ...
                <div class="pt-4 flex items-center gap-6">
                    ...
                    <span class="text-[10px] font-bold uppercase tracking-widest text-emerald-800">REPAIR MODE</span>
                </div>
            </form>
        </div>
    </div>
