...
<div class="mb-6 flex justify-between items-end">
    <div>
        ...
        <h2 class="text-xl font-bold text-white tracking-tight mt-2">Edit Dealer: {{ $dealer->user->name }}</h2>
        <p class="text-sm text-gray-400 mt-1">Update dealer profile and contact information.</p>
    </div>
    <div class="flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg">
        ...
        <span class="badge {{ $dealer->status === 'active' ? 'badge-green' : 'badge-gray' }}">{{ ucfirst($dealer->status) }}</span>
    </div>
</div>

<div class="max-w-4xl">
    <form action="{{ route('admin.dealers.update', $dealer) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="card p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $dealer->user->name) }}" required
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="business_name" value="{{ old('business_name', $dealer->business_name) }}" required
                        class="w-full @error('business_name') border-red-500 @enderror">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="phone" value="{{ old('phone', $dealer->phone) }}" required
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="gst_number" value="{{ old('gst_number', $dealer->gst_number) }}"
                        class="w-full @error('gst_number') border-red-500 @enderror">
                    @error('gst_number') <p class="text-xs text-red-500">{{ $message }}</p> @enderror
                </div>

                {{-- Status --}}
                <div class="space-y-2">
                    <label class="text-sm font-medium text-gray-300">Account Status</label>
                    <select name="status" class="w-full" required>
                        <option value="active" {{ $dealer->status === 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ $dealer->status === 'inactive' ? 'selected' : '' }}>Inactive / Deactivated</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <textarea name="address" rows="3" required
                        class="w-full @error('address') border-red-500 @enderror">{{ old('address', $dealer->address) }}</textarea>
                    @error('address') <p class="text-xs text-red-500">{{ $message }}</p> @enderror
                </div>
            </div>
        </div>

        <div class="card p-6 border-l-2 border-indigo-500/50">
            <h3 class="text-lg font-semibold text-white mb-6 border-b border-white/5 pb-2 flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="email" name="email" value="{{ old('email', $dealer->user->email) }}" required
                ...
                <div class="space-y-2" x-data="{ show: false }">
                    <label class="text-sm font-medium text-gray-300">Change Password (Leave blank to keep current)</label>
                    <div class="relative">
                        <input :type="show ? 'text' : 'password'" name="password" 
                            class="w-full @error('password') border-red-500 @enderror" placeholder="New password">
        ...
        <div class="flex items-center justify-end gap-3 pt-4">
            ...
            <button type="submit" class="btn-primary px-10">Update Dealer Profile</button>
        </div>
    </form>
</div>
@endsection
