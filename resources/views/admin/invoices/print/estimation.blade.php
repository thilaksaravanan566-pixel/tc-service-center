...
<body class="bg-gray-50 py-10 px-4 relative">
    <!-- Watermark -->
    <div class="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.03] z-0 overflow-hidden no-print" aria-hidden="true">
        <span class="text-[15rem] font-black uppercase text-gray-900 rotate-[-45deg] tracking-widest leading-none">ESTIMATE</span>
    </div>
    <div class="print-container watermark-print absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.03] z-0 overflow-hidden hidden" aria-hidden="true">
        <span class="text-[15rem] font-black uppercase text-black rotate-[-45deg] tracking-widest leading-none">ESTIMATE</span>
    </div>
    <style>
        @media print {
    ...
    <div class="print-container max-w-4xl mx-auto bg-white p-12 shadow-2xl relative">
        ...
        <div class="flex justify-between items-start mb-8 border-b-2 pb-6 theme-border">
            ...
            <div class="text-right">
                <h2 class="text-4xl font-black uppercase text-sky-600 mb-2">ESTIMATION</h2>
                ...
                <p class="text-[10px] text-rose-500 font-bold uppercase mt-1">Valid Until: {{ $invoice->valid_until->format('d M Y') }}</p>
                @endif
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-gray-700">
                    ...
                    <tr class="border-b border-gray-200 {{ $loop->even ? 'bg-gray-50' : 'bg-white' }}">
                        ...
                        <td class="py-4 px-4 relative z-10">
                        ...
                        <td class="py-4 px-4 text-center font-medium relative z-10">{{ $item->quantity }}</td>
        ...
        <div class="flex justify-between items-end mb-12">
            ...
            <div class="w-1/2 pr-8">
                ...
                <div class="mt-6">
                    <img src="https://api.qrserver.com/v1/create-qr-code/?size=80x80&data={{ urlencode(route('admin.invoices.show', $invoice->id)) }}" alt="QR Code" class="w-16 h-16 opacity-80">
                    <div class="text-[8px] text-gray-400 mt-1 uppercase tracking-widest font-bold">Scan Origin</div>
                </div>
            </div>
            ...
            <div class="w-2/5 rounded-lg border border-gray-200 overflow-hidden">
                <div class="bg-gray-50 p-4 border-b border-gray-200">
                    ...
                    @if($invoice->discount > 0)
                        <div class="flex justify-between mb-2 text-red-500"><span class="font-medium">Discount</span><span class="font-bold">-₹{{ number_format($invoice->discount, 2) }}</span></div>
                    @endif
                </div>
